...
this.disclosureColumnIdentifier=columnIdentifier;var cell=createElement("th");cell.className=columnIdentifier+"-column";cell.columnIdentifier=columnIdentifier;this._headerTableHeaders[columnIdentifier]=cell;var div=createElement("div");if(column.titleDOMFragment)
...
{this._currentResizer=null;this._saveColumnWeights();this.dispatchEventToListeners(WebInspector.DataGrid.Events.ColumnsResized);},ColumnResizePadding:24,CenterResizerOverBorderAdjustment:3,__proto__:WebInspector.Widget.prototype}
...
{this.isCreationNode=false;},__proto__:WebInspector.DataGridNode.prototype};WebInspector.ViewportDataGrid=function(columnsArray,editCallback,deleteCallback,refreshCallback,contextMenuCallback)
{WebInspector.DataGrid.call(this,columnsArray,editCallback,deleteCallback,refreshCallback,contextMenuCallback);this._scrollContainer.addEventListener("scroll",this._onScroll.bind(this),true);this._scrollContainer.addEventListener("mousewheel",this._onWheel.bind(this),true);this._visibleNodes=[];this._flatNodes=null;this._updateScheduled=false;this._inline=false;this._wheelTarget=null;this._hiddenWheelTarget=null;this._stickToBottom=false;this._atBottom=true;this._lastScrollTop=0;this.setRootNode(new WebInspector.ViewportDataGridNode());}
...
return;this._updateScheduled=true;this.element.window().requestAnimationFrame(this._update.bind(this));},renderInline:function()
...
return{topPadding:0,bottomPadding:0,visibleNodes:nodes,offset:0};var size=nodes.length;var i=0;var y=0;for(;i<size&&y+nodes[i].nodeSelfHeight()<scrollTop;++i)
...
bottomPadding+=nodes[i].nodeSelfHeight();return{topPadding:topPadding,bottomPadding:bottomPadding,visibleNodes:nodes.slice(start,end),offset:start};},_contentHeight:function()
...
{this._updateScheduled=false;var clientHeight=this._scrollContainer.clientHeight;var scrollTop=this._scrollContainer.scrollTop;var currentScrollTop=scrollTop;var maxScrollTop=Math.max(0,this._contentHeight()-clientHeight);if(this._stickToBottom&&this._atBottom)
...
for(var i=0;i<this._visibleNodes.length;++i){var oldNode=this._visibleNodes[i];if(!visibleNodesSet.has(oldNode)){var element=oldNode.element();if(element===this._wheelTarget)
...
{WebInspector.DataGridNode.call(this,data,false);this._stale=false;}
...
{child.deselect();this.children.remove(child,true);if(child.previousSibling)
child.previousSibling.nextSibling=child.nextSibling;if(child.nextSibling)
child.nextSibling.previousSibling=child.previousSibling;if(!this.children.length)
this.hasChildren=false;if(this._expanded)
this.dataGrid.scheduleUpdateStructure();},removeChildren:function()
{for(var i=0;i<this.children.length;++i)
this.children[i].deselect();this.children=[];if(this._expanded)
this.dataGrid.scheduleUpdateStructure();},collapse:function()
{if(!this._expanded)
...
{if(this._expanded)
return;this._expanded=true;if(this._element)
this._element.classList.add("expanded");this.dataGrid.scheduleUpdateStructure();},willAttach:function(){},attached:function()
{return!!(this._element&&this._element.parentElement);},refresh:function()
...
{var parentNode=this.rootNode();parentNode.insertChild(node,parentNode.children.upperBound(node,this._sortingFunction));},sortNodes:function(comparator,reverseMode)
{this._sortingFunction=WebInspector.SortableDataGrid.Comparator.bind(null,comparator,reverseMode);var children=this._rootNode.children;children.sort(this._sortingFunction);for(var i=0;i<children.length;++i)
children[i].recalculateSiblings(i);this.scheduleUpdateStructure();},__proto__:WebInspector.ViewportDataGrid.prototype}
WebInspector.SortableDataGridNode=function(data)
{WebInspector.ViewportDataGridNode.call(this,data);}
WebInspector.SortableDataGridNode.prototype={__proto__:WebInspector.ViewportDataGridNode.prototype};WebInspector.ShowMoreDataGridNode=function(callback,startPosition,endPosition,chunkSize)
...
return space;index%=space.count;return space.min+Math.floor(index/space.count*(space.max-space.min));}}
...
{this._isDragging=false;},_hideRangeSelection:function()
...
var colors=Object.keys(colorBuckets);for(var c=0;c<colors.length;++c){var color=colors[c];context.fillStyle=color;context.strokeStyle=color;var indexes=colorBuckets[color];context.beginPath();for(var i=0;i<indexes.length;++i){var entryIndex=indexes[i];var entryStartTime=entryStartTimes[entryIndex];var barX=this._timeToPositionClipped(entryStartTime);var barRight=this._timeToPositionClipped(entryStartTime+entryTotalTimes[entryIndex])+1;var barWidth=barRight-barX;var barLevel=entryLevels[entryIndex];var barY=this._levelToHeight(barLevel);if(isNaN(entryTotalTimes[entryIndex])){context.moveTo(barX+this._markerRadius,barY+barHeight/2);context.arc(barX,barY+barHeight/2,this._markerRadius,0,Math.PI*2);markerIndices[nextMarkerIndex++]=entryIndex;}else{context.rect(barX,barY,barWidth,barHeight-1);if(barWidth>minTextWidth||this._dataProvider.forceDecoration(entryIndex))
...
return;var barWidth=barRight-barX;var barCenter=barX+barWidth/2;barWidth=Math.max(barWidth,elementMinWidth);barX=barCenter-barWidth/2;var barY=this._levelToHeight(timelineData.entryLevels[entryIndex])-this._scrollTop;var style=element.style;style.left=barX+"px";style.top=barY+"px";style.width=barWidth+1+"px";style.height=this._barHeight-1+"px";this.contentElement.appendChild(element);},_timeToPositionClipped:function(time)
{return Math.max(0,this._timeToPosition(time));},_timeToPosition:function(time)
...
{var infoTable=createElementWithClass("table","info-table");for(var i=0;i<entryInfo.length;++i){var row=infoTable.createChild("tr");row.createChild("td","title").textContent=entryInfo[i].title;row.createChild("td").textContent=entryInfo[i].text;}
...
{this._offsetWidth=this.contentElement.offsetWidth-(WebInspector.isMac()?0:this._vScrollElement.offsetWidth);this._offsetHeight=this.contentElement.offsetHeight;},_onScroll:function()
...
WebInspector.installDragHandle(dividersLabelBarElement,this._startWindowDragging.bind(this),this._windowDragging.bind(this),null,"-webkit-grabbing","-webkit-grab");this.windowLeft=0.0;this.windowRight=1.0;this._parentElement.addEventListener("mousewheel",this._onMouseWheel.bind(this),true);this._parentElement.addEventListener("dblclick",this._resizeWindowMaximum.bind(this),true);this._overviewWindowElement=parentElement.createChild("div","overview-grid-window");this._overviewWindowElement.appendChild(WebInspector.Widget.createStyleElement("ui_lazy/overviewGrid.css"));this._overviewWindowBordersElement=parentElement.createChild("div","overview-grid-window-rulers");parentElement.createChild("div","overview-grid-dividers-background");this._currentPositionElement=parentElement.createChild("div","overview-grid-current-position");this._currentPositionArea=parentElement.createChild("div","overview-grid-window-area");this._currentPositionArea.addEventListener("mousemove",this._onMouseMove.bind(this),true);this._currentPositionArea.addEventListener("mouseout",this._hideCurrentPosition.bind(this),true);this._leftResizeElement=parentElement.createChild("div","overview-grid-window-resizer");this._leftResizeElement.style.left=0;WebInspector.installDragHandle(this._leftResizeElement,this._resizerElementStartDragging.bind(this),this._leftResizeElementDragging.bind(this),null,"ew-resize");this._rightResizeElement=parentElement.createChild("div","overview-grid-window-resizer overview-grid-window-resizer-right");this._rightResizeElement.style.right=0;WebInspector.installDragHandle(this._rightResizeElement,this._resizerElementStartDragging.bind(this),this._rightResizeElementDragging.bind(this),null,"ew-resize");this._setEnabled(true);}
...
return;this._enabled=enabled;this._currentPositionArea.style.cursor=enabled?"text":"";if(!enabled)
this._hideCurrentPosition();},_hideCurrentPosition:function()
{this._currentPositionElement.style.visibility="hidden";},_onMouseMove:function(event)
{if(!this._enabled)
return;var x=event.offsetX+event.target.offsetLeft;this._currentPositionElement.style.left=x+"px";this._currentPositionElement.style.visibility="visible";},_resizerElementStartDragging:function(event)
{if(!this._enabled)
...
{this._leftCurtainElement.style.width=gapOffset+"px";this._leftCurtainElement.classList.remove("hidden");this._rightCurtainElement.style.left=(gapOffset+gapWidth)+"px";this._rightCurtainElement.classList.remove("hidden");},setScrollTop:function(scrollTop)
